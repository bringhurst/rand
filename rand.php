...
<html>
  ...
  <body>
    ...
    <p>WARNING: Your password may NOT be secure if you do not use all 14 characters (symbol group size is only 64).</p>
    <?php
      echo "<b>available pool = ";
      $entropy_avail = system('cat /proc/sys/kernel/random/entropy_avail');
      echo "</b>";
      if($entropy_avail < 150) {
        echo "<p><font color=red>WARNING: Not enough entropy is available. Please try again in a few seconds.</font></p>";
      } else {
        echo "<pre><p>START OF PASSWORD LIST</p>";
        system("/usr/bin/openssl rand -base64 120|/bin/cut -c0-14");
